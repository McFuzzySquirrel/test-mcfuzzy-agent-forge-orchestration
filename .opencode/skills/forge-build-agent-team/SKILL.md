...
| Mode | Signals | Action |
|------|---------|--------|
| **Full Build** | Complete PRD with Overview, Technical Architecture, Implementation Phases. No `.agent.md` files in `.opencode/agents/` beyond forge templates. | Continue with Steps 1–8 below. |
...
| **Feature Increment** | Document is a Feature PRD (has "Feature Overview", "Agent Impact Assessment"). Existing `.agent.md` files in `.opencode/agents/`. | Load `references/feature-increment-mode.md` and follow its process. |
...
Create each agent file at `.opencode/agents/{agent-name}.agent.md`:
...
Create each skill file at `.opencode/skills/{skill-name}/SKILL.md`.
